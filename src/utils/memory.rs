...
pub fn estimate_model_memory(
    ...
    // attention: Q K V O projections = 4 x d_model x d_model
    let attn_weights = 4 * d_model * d_model;
    // feedforward: gate, up, down = 3 x d_model x d_ff
    ...
    let weight_bytes      = (total_weight_bits + 7) / 8;

    // embedding table stays fp16
    let embed_bytes = vocab_size * d_model * 2;

    // layer norm gains per layer (fp16)
...
mod tests {
    ...
    fn memory_estimate_7b_ternary() {
        // 7B ternary should land between 1 GB and 10 GB
        let bytes = estimate_model_memory(
            32, 4096, 11008, 32000, QuantMode::Ternary
        );
